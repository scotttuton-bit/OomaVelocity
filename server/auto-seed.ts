...
export async function autoSeedIfEmpty() {
  ...
  if (Number(count) > 0) {
    console.log(`Database has ${count} metrics, skipping auto-seed.`);
    return;
  }
  ...
  console.log('Auto-seed: Created', createdDevices.length, 'devices');
  ...
  function generateRealisticValue(base: number, variance: number, hourOfDay: number, dayOfWeek: number, trendOffset: number): number {
  ...
  for (const profile of deviceProfiles) {
    ...
    for (let t = now - NINETY_DAYS_MS; t <= now; t += getInterval(now - t)) {
      ...
      if (rollingDown.length > ROLLING_WINDOW) { rollingDown.shift(); rollingUp.shift(); rollingPing.shift(); }
      ...
      metricsData.push({
        timestampIso: date, downloadMbps: download, uploadMbps: upload, pingMs: ping,
        avgDownloadMbps: avgDown, avgUploadMbps: avgUp, avgPingMs: avgPing,
        alertFlags, deviceId: profile.id, location: profile.location
      });
    }
  }
  ...
  for (let i = 0; i < metricsData.length; i += BATCH_SIZE) {
    await db.insert(networkMetrics).values(metricsData.slice(i, i + BATCH_SIZE));
  }
  console.log('Auto-seed: Created', metricsData.length, 'metrics');

  const nycInactive = findDevice('NYC-DC-AGT-01');
  const denOffline = findDevice('DEN-DC-AGT-01');
  const sfDevice = findDevice('SF-HQ-RTR-01');
  const chiDevice = findDevice('CHI-OFF-RTR-01');

  await db.insert(alerts).values([
    { title: 'High Ping Detected', description: 'SF-HQ-RTR-01 ping exceeded 2x average', severity: 'warning' as const, deviceId: sfDevice.id, acknowledged: false },
    { title: 'Speed Drop Alert', description: 'CHI-OFF-RTR-01 download dropped below 50%', severity: 'error' as const, deviceId: chiDevice.id, acknowledged: false },
    { title: 'Device Offline', description: 'DEN-DC-AGT-01 offline for 48+ hours', severity: 'error' as const, deviceId: denOffline.id, acknowledged: false },
    { title: 'Agent Disconnected', description: 'NYC-DC-AGT-01 stopped reporting', severity: 'warning' as const, deviceId: nycInactive.id, acknowledged: false },
  ]);
  console.log('Auto-seed: Created 4 alerts');
